...
class ShallowHandModel:
    def __init__(self, device="cpu"):
        ...
        def build_mesh_recurse(body):
            if(len(body.link.visuals) > 0):
                ...
                fingertip_keypoints = torch.tensor(fingertip_points[link_name], dtype=torch.float32, device=device).reshape(-1, 3) if fingertip_points is not None else None
                ...
                self.mesh[link_name] = dict(
                    ...
                    fingertip_keypoints=fingertip_keypoints,
    ...
    def __call__(self, hand_pose, object_pc=None, plane_parameters=None, with_fingertip_keypoints=False, with_meshes=False, with_surface_points=False, with_contact_candidates=False, with_penetration_keypoints=False, with_penetration=False):
        ...
        if with_fingertip_keypoints:
            hand['fingertip_keypoints'] = get_points('fingertip_keypoints')
