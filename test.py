...
class Tester(object):
    def __init__(self, cfg: EasyConfig) -> None:
        ...
        self.save_root = osp.join(cfg.save_root, "test_results", f"epoch_{self.epoch}_{cfg.set}")
    ...
    def test_one_iter(self, data: Dict):
        ...
        matched_results = []
        for _ in range(1):
            _, _, predictions = self.model.forward_test(data)
            ...
            matched_label = data["matched"]["hand_model_pose"]  # (K, 28)
    ...
    def test(self) -> float:
        ...
        with tqdm(
            ...
            for data_dict in self.test_loader:
                ...
                raw_results += _raw_result
                matched_results += _matched_result
                progress_bar.update()
        # save test results
        raw_results_path = osp.join(self.save_root, "raw_results.json")

        raw_results = self.process_out(raw_results)
...
def parse_args(arg_str: Optional[str] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--train_set", action="store_true", help="inference on train set")
