...
class Trainer(object):
    def __init__(self, cfg: EasyConfig) -> None:
        ...
        self.validate_freq = cfg.validate_freq
        self.frozen_clip = cfg.frozen_clip
        self.resume = False
        ...
        self.saved_models = [deepcopy({"save_path": "", "performance": -float('inf')}) for _ in range(cfg.save_top_n)]
    ...
    def _record_cfg(self, cfg: EasyConfig, display: bool = True) -> None:
        ...
        with open(osp.join(cfg_dir, "train_config.yaml"), "w") as wf:
            wf.write(yaml.dump(cfg.dict(), indent=2, allow_unicode=True, sort_keys=False))
    ...
    def _train_one_epoch(self, epoch: int) -> None:
        ...
        if self.frozen_clip:
            try:
                self.model.eps_model.language_model.freeze()
            except:
                self.model.language_encoder.freeze()
    ...
    def _validate(self, epoch: int) -> float:
        ...
        with tqdm(
            ...
            for data_dict in self.val_loader:
                ...
                data_dict["save_root"] = self.cfg.save_root
                ##
                data_dict = self._recursive_to_device(data_dict)
                loss, loss_dict, _ = self.model(data_dict)
